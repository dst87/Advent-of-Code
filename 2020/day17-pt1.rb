...
# to make processing easier. [z][x][y]
...
layout = [expanded_layer]
i = 1
while i <= 6
	layout.unshift($empty_layer)
	layout.push($empty_layer)
	i += 1
end

# Method to process the input, returning the new layout.

def active_neighbours_count(z, y, x, layout)
	adjacent_states = []
	(z-1..z+1).each do |zz|
		next if zz < 0 || zz > 12
		(x-1..x+1).each do |xx|
			...
			(y-1..y+1).each do |yy|
				next if yy < 0 || yy > 19
				next if (z == zz) && (y == yy) && (x == xx)
				adjacent_states << layout[zz][yy][xx]
			end
		end
	end
...
def cycle(input)
	new_layout = []
	input.each_with_index do |layer, z|
		new_layout[z] ||= []
		layer.each_with_index do |row, y|
			new_layout[z][y] ||= []
			row.each_with_index do |state, x|
				new_layout[z][y][x] ||= []
				count = active_neighbours_count(z, y, x, input)
